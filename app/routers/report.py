...
from app.utils.tasks import enqueue_report_analysis
...
def create_report(
    ...
    try:
        # 定义不需要AI分析的检查类型
        # 1: 眼表拍照（常规检查）
        # 4: FBUT泪膜破裂时间（视频，不需要AI分析）
        # 7: 脂质层分析（视频，不需要AI分析）
        no_ai_analysis_types = {1, 4, 7}
        
        # 验证检查类型
        if report.check_type not in no_ai_analysis_types:

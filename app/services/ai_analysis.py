...
CHECK_TYPE_GUIDE_PROMPTS = {
    ...
    6: "这是一张眼部图像，请评估以下几点：是否有磷屑，是否有油脂堵塞，是否有气泡，是否有分泌物，字数不超过60个字。",
...
CHECK_TYPE_EXPERT_PROMPTS = {
    ...
    4: "结合左右眼TBUT评估结果，分析泪膜稳定性并提供干眼管理建议，只回复纯文本格式不用markdown格式",
    5: "根据眼红指数结果，评估眼表充血程度并给出缓解建议，只回复纯文本格式不用markdown格式",
    6: "根据睑缘和睫毛的健康分析结果，给出专业的眼科分析并提供护理建议，只回复纯文本格式不用markdown格式",
